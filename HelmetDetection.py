...
=======
        ...
        image, ops, pcount = detectObject(cnn_model, cnn_layer_names, image_height, image_width, image, label_colors, class_labels,indexno)
        if ops == 1:
...
=======
    ...
    for i in indices:
        ...
        if my_class == unknown_class:
            count_person += 1
    flag = False        
    print(str(frame_count_out)+" "+str(count_person)+" "+str(option))
    if count_person == 0 and option == 1:
        cv.putText(frame, "Helmet Not detected", (10, 50), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0,255,0), 2)
        if pcount >= 3:
            cv.putText(frame, "Triple Riding Detected", (10, 100), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0,255,0), 2)
        flag = True
    if count_person >= 1 and option == 0:
        ...
        cv.imshow('img',frame)
        cv.waitKey(50)
    if pcount >= 3 and flag == False:
        cv.putText(frame, "Triple Riding & No Helmet Detected", (10, 100), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0,255,0), 2)
        frame = cv.resize(frame,(800,500))
        cv.imshow('img',frame)
        cv.waitKey(50)    
...
font = ('times', 16, 'bold')
title = Label(main, text='Helmet Detection and Triple Riding Detection using CNN and YOLOV', justify=LEFT)
